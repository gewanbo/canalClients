...
/**
  ...
  */
class InteractiveTagIndex extends ContentIndex with Pipeline with Logging {
    ...
    def init(): Unit ={
        // Set content type 3 (Interactive)
        _contentType = 3
    }

    override def process(changedRow: RowChange): Unit = {

        log.info("****** Interactive tag index pipeline ******")
        ...
        rowData.foreach(data => {
            ...
            if(eventType == EventType.INSERT) {
                ...
                val cShortLead = checkLead(afterCols.find(_.getName == "cshortleadbody").head.getValue)
                val cLongLead = checkLead(afterCols.find(_.getName == "clongleadbody").head.getValue)
                val pubDate = afterCols.find(_.getName == "pubdate").head.getValue
                ...
                val afterCols = data.getAfterColumnsList.asScala
                val itemId = afterCols.find(_.getName == "id").head.getValue
                val itemCheadLine = checkLead(afterCols.find(_.getName == "cheadline").head.getValue)
                ...
                val beforeShortLead = checkLead(beforeCols.find(_.getName == "cshortleadbody").head.getValue)
                val beforeLongLead = checkLead(beforeCols.find(_.getName == "clongleadbody").head.getValue)

                val afterCols = data.getAfterColumnsList.asScala
                val afterId = afterCols.find(_.getName == "id").head.getValue
                val afterCheadLine = afterCols.find(_.getName == "cheadline").head.getValue
                val afterTag = afterCols.find(_.getName == "tag").head.getValue
                val afterShortLead = checkLead(afterCols.find(_.getName == "cshortleadbody").head.getValue)
                val afterLongLead = checkLead(afterCols.find(_.getName == "clongleadbody").head.getValue)
                val pubDate = afterCols.find(_.getName == "pubdate").head.getValue
    ...
    def delProperties(contentType: Int, contentId: String): Boolean ={
        ...
        try {
            ...
            if(contentId == "") {
                throw new Exception("Must specify a interactive id.")
            }
    ...
    private def checkLead(lead: String): String ={
        var validLead = ""
        if(lead.length < 200){
            if(!lead.contains("|") && !lead.contains("\n")){
                validLead = lead.replaceAll("'", "\\'")
            }
        }
        validLead
    }
