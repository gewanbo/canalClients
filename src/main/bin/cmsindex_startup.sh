...
base=${bin_abs_path}/..
easy_conf=${base}/conf/cmsindex.xml
logback_configurationFile=${base}/conf/logback.xml
export LANG=en_US.UTF-8
export BASE=${base}

if [ -f ${base}/bin/cmsindex.pid ] ; then
	echo "found cmsindex.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
EASY_OPTS="-DappName=cmsindex -Dlogback.configurationFile=$logback_configurationFile -Deasy.conf=$easy_conf"
